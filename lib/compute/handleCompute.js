...
function isValidExecCheck(execCheck) {
  if (!execCheck || typeof execCheck !== "object" || Array.isArray(execCheck)) {
    return false;
  }

  if (typeof execCheck.status !== "string" || !VALID_STATUS.has(execCheck.status)) {
    return false;
  }
  ...
  return (
    typeof execCheck.reason === "string" &&
    VALID_REASON.has(execCheck.reason)
  );
}
...
export async function handleCompute(input) {
  ...
  const result = await computeRunner(prepared, cleanInput, schema);
