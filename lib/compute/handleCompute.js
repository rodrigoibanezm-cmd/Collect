...
export async function handleCompute(input) {
  ...
  const cleanInput = {
    ...
    analysis_type: typeof input?.analysis_type === "string" ? input.analysis_type.trim().toLowerCase() : "",
    output_mode: typeof input?.output_mode === "string" ? input.output_mode.trim().toLowerCase() : ""
  };
  ...
  try {
    ...
    const result = await computeDiagnostico(prepared, cleanInput);
