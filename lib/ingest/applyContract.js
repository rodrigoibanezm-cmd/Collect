...
export function applyContract(data, schema, codebook) {
  ...
  const declaredFieldDef = ic?.structure?.fields?.[varName];

  const codebookVar = codebook?.variables?.[varName];
  if (!codebookVar) {
    throw new Error(`contract_error:missing_codebook_variable:${varName}`);
  }

  const fieldDef = declaredFieldDef || {
    type: "integer",
  ...
  const labelLookup = codebookVar?.value_labels || {};
