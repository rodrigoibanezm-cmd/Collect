...
export function applyContract(data, schema, codebook) {
  ...
  for (const row of data) {
    ...
    let value = coerceValue(row?.[varName]);
  ...
  if (segmentationConfig?.enabled === true) {
    ...
    for (const segDef of segmentDefs) {
      ...
      for (const row of data) {
        let segValue = coerceValue(row?.[segVar]);
      ...
      for (const [segKey, rows] of segmentBuckets.entries()) {
        const segLabel =
          typeof segLabels[segKey] === "string" ? segLabels[segKey] : segKey;
        ...
        for (const row of rows) {
          ...
          const value = coerceValue(row?.[varName]);

          if (!isValidTargetValue(value)) continue;
        ...
        const minN = Number.isFinite(segDef?.min_n) ? segDef.min_n : 0;

        if (segValidBase < minN) {
          continue;
        }

        const segOptions = Array.from(segCounts.values())
          .map((item) => ({
            ...item,
            share: segValidBase > 0 ? item.n / segValidBase : 0
          }))
          .sort((a, b) => b.n - a.n);
